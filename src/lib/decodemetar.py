...
def parse_report(line, my_default_values):
    """Parses a METAR report string using metar_parsers dict. Returns a dictionary.

    >>> cycle2txt.parse_report("KFDW 242355Z AUTO -RA SCT080 17/12 ", {'time': '2012-02-25 00:37:00'})
    {'code': ('KFDW',), 'temperature': ('17',), 'time': '2012-02-25 00:37:00'}

    >>> cycle2txt.parse_report("KFDW 242355Z AUTO 24009G18KT 10SM -RA SCT080 17/12 A2974 RMK AO1", {'time': '2012-02-25 00:37:00'})
    ...
    for i in columns :
        if values.get(i) :
            ...
            if value <> '\N' and metar_parsers.get(i) :
                value = metar_parsers[i][1](field_values[i])
            list.append(value)
    return "\t".join(list)
...
if __name__ == '__main__':
    for line in fileinput.input():
        ...
        # if there was a timestamp in previous line, read a METAR report
        if 'time' in field_values.keys():
            field_values = parse_report(line,field_values)
